...
export async function geminiTranslate(text, from, to, modelName = 'gemini-1.5-flash', providedApiKey = null) {
    if (!providedApiKey) {
        console.warn("Gemini API Key is not provided. Skipping Gemini translation.");
        return null;
    }

    try {
        const genAIInstance = new GoogleGenerativeAI(providedApiKey);
...
export async function geminiRefine(original, translated, from, to, modelName = 'gemini-1.5-flash', providedApiKey = null) {
    if (!providedApiKey) {
        console.warn("Gemini API Key is not provided. Skipping Gemini refinement.");
        return translated;
    }
...
export async function geminiSTT(audioBase64, languageCode = 'en-US', modelName = DEFAULT_STT_MODEL, providedApiKey = null) {
    ...
    try {
        ...
        for (const apiVersion of apiVersions) {
            const cacheKey = `${apiVersion}:${hashKey(providedApiKey)}`;
            ...
            const directUrl = `https://generativelanguage.googleapis.com/${apiVersion}/models/${modelIdToUse}:generateContent?key=${providedApiKey}`;
            ...
            if (directResponse.status === 429) {
                if (modelIdToUse !== 'gemini-1.5-flash' && modelIdToUse !== 'gemini-1.5-flash-latest') {
                    ...
                    const retryUrl = `https://generativelanguage.googleapis.com/${apiVersion}/models/${modelIdToUse}:generateContent?key=${providedApiKey}`;
            ...
            if (directResponse.status === 404) {
                const models = await listModels(providedApiKey, apiVersion);
                if (models.length) {
                    const sample = models
                        .slice(0, 8)
